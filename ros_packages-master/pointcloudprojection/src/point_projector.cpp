...
std::vector<cv::Vec3f>
point_projector::project_points(const std::vector<cv::Vec3f> &point_cloud) {
  assert(transform_matrix.rows > 0 && intrinsics_matrix.rows > 0);
  ...
  for (const auto &point : point_cloud) {
    ...
    cv::Vec3f vector{result.at<float>(0), result.at<float>(1),
                     result.at<float>(2)};
