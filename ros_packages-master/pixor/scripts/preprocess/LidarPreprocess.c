...
{	
    ...
    void createTopViewMaps2(const void * indatav, float *data, int32_t num){
    	float * data_cube = (float *) indatav;
	float *px = data+0;
	float *py = data+1;
	float *pz = data+2;
	float *pr = data+3;
	//int32_t num = sizeof(data)/sizeof(float*);
	std::cout << "THE DATA PASSED IN IS SIZEOF " << num << std::endl; 
