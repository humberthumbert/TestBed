...
class TimeStreamer:
	def __init__(self):
		...
		self.subscriber3 = rospy.Subscriber("/pixel3/image_raw", Image, self.callback, "2", queue_size=1)
		self.publisher_pixel3 = rospy.Publisher("/pixel3/image_timed", Image, queue_size=1)
	...
	def callback(self, ros_data, args):
		...
		self.publisher_pixel3.publish(msg)

	def getTimers(self):
		HOST = 'pixel3'  # The server's hostname or IP address
...
def main():
	rospy.init_node('timer_3', anonymous=True)
	ts = TimeStreamer()
	ts.getTimers()
