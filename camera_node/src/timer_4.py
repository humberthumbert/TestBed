...
class TimeStreamer:
	def __init__(self):
		...
		self.subscriber4 = rospy.Subscriber("/pixel4/image_raw", Image, self.callback, "2", queue_size=1)
		self.publisher_pixel4 = rospy.Publisher("/pixel4/image_timed", Image, queue_size=1)
	...
	def callback(self, ros_data, args):
		...
		self.publisher_pixel4.publish(msg)

	def getTimers(self):
		HOST = 'pixel4'  # The server's hostname or IP address
...
def main():
	rospy.init_node('timer_4', anonymous=True)
	ts = TimeStreamer()
	ts.getTimers()
